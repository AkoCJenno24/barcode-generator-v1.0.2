...
interface ReportsModalProps {
  ...
  onDeletePriceUpdate?: (priceUpdate: PriceUpdateItem) => Promise<{ success: boolean; error: string | null }>;
}
...
export const ReportsModal: React.FC<ReportsModalProps> = ({
  ...
  const [confirmDeleteTarget, setConfirmDeleteTarget] = useState<PriceUpdateItem | null>(null);
  ...
  const handleOpenDeleteConfirm = (item: PriceUpdateItem) => {
    setConfirmDeleteTarget(item);
  };

  const handleConfirmDeletePriceUpdate = async () => {
    ...
    try {
      if (onDeletePriceUpdate) {
        await onDeletePriceUpdate(target);
      }
      setPriceUpdates((prev) => prev.filter((p) => p !== target && p.id !== target.id));
    } catch (err) {
      console.error('Failed to revert/delete price update line:', err);
    } finally {
      setDeletingId(null);
  ...
  const downloadPriceUpdateCsv = () => {
    ...
    const headers = ['Item Code', 'Item Name', 'Old Unit Cost', 'New Unit Cost', 'Old MRP', 'New MRP', 'Vatted', 'Updated At'];
    ...
    const rows = priceUpdates.map((item) => {
      const updatedDateStr = item.createdAt
        ? new Date(item.createdAt).toLocaleString()
        : '';
      const vattedStr = item.isVatted !== undefined ? (item.isVatted ? 'Yes' : 'No') : '';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
                  ...
                  <table className="w-full text-left text-xs text-slate-700 border-collapse">
                    <thead>
                      <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-bold uppercase text-[11px] tracking-wider">
                        ...
                        <th className="px-4 py-3 text-center">Vatted</th>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-center">
                            <button
                              ...
                              className="inline-flex items-center justify-center p-1.5 rounded-lg text-rose-600 hover:text-rose-800 hover:bg-rose-50 border border-transparent hover:border-rose-200 transition-colors cursor-pointer disabled:opacity-40"
                              title="Delete price update line & revert item in catalog"
                            >
                              {deletingId === item.id ? (
      ...
      {/* Confirmation Dialog overlay for deleting price update line & reverting catalog */}
      {confirmDeleteTarget && (
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-slate-900">
                  Delete & Revert Price Update?
                </h3>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Are you sure you want to delete this price update line for item{' '}
                  <strong className="text-slate-900 font-mono">{confirmDeleteTarget.itemCode}</strong>?
                  This will remove the record from database and revert the catalog item.
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2 text-xs">
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                Values to Revert in Catalog:
              </div>
              <div className="grid grid-cols-2 gap-2 text-slate-700">
                <div>
                  <span className="text-slate-400">Revert MRP:</span>{' '}
                  <strong className="text-emerald-700 font-mono">{confirmDeleteTarget.oldMrp}</strong>
                </div>
                <div>
                  <span className="text-slate-400">Revert Unit Cost:</span>{' '}
                  <strong className="text-slate-900 font-mono">{confirmDeleteTarget.oldUnitCost || '0'}</strong>
                </div>
                <div className="col-span-2">
                  <span className="text-slate-400">Revert Vatted:</span>{' '}
                  <strong className="text-slate-900">
                    {confirmDeleteTarget.oldIsVatted !== undefined
                      ? confirmDeleteTarget.oldIsVatted
                        ? 'Yes'
                        : 'No'
                      : confirmDeleteTarget.isVatted !== undefined
                      ? confirmDeleteTarget.isVatted
                        ? 'No'
                        : 'Yes'
                      : 'Unchanged'}
                  </strong>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                type="button"
                onClick={handleConfirmDeletePriceUpdate}
                disabled={!!deletingId}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-50 transition-colors cursor-pointer shadow-xs"
              >
                {deletingId ? (
                  <>
